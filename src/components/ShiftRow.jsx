...
export default function ShiftRow({
  ...
  // Active focused field for rendering inline suggestions
  ...
  return (
    <div className={`rounded-2xl border transition-all duration-200 ${cardBorderAndBg}`}>
      ...
      <div className="md:hidden p-3.5 space-y-3">
        ...
        {!isOff ? (
          <div className="space-y-2.5 pt-1">
            ...
            {/* 🌟 IN-FLOW LIQUID GLASS QUICK-TIME DOCK (Zero Overflow, Zero Click-Through) */}
            {activeSuggestionConfig && (
              <div className="p-2 rounded-2xl backdrop-blur-xl bg-cyan-500/10 dark:bg-slate-950/80 border border-cyan-500/30 shadow-sm animate-in fade-in zoom-in-95 duration-200">
                <div className="flex items-center justify-between gap-1 mb-1.5 text-[10px] font-bold text-cyan-800 dark:text-cyan-300">
                  ...
                  <span className="text-[9px] text-slate-400 font-normal">Tap to auto-fill & advance</span>
                </div>
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto whitespace-nowrap scrollbar-none pb-0.5">
                  {activeSuggestionConfig.times.map((time) => (
                    <button
                      key={time}
                      type="button"
                      onClick={() => handleSelectSuggestionTime(time)}
                      className="px-2.5 py-1 text-xs font-mono font-bold text-slate-800 dark:text-slate-100 bg-white/90 dark:bg-slate-800/90 hover:bg-gradient-to-r hover:from-cyan-500 hover:to-emerald-500 hover:text-white dark:hover:text-white rounded-xl border border-white/80 dark:border-slate-700 shadow-sm active:scale-95 transition-all duration-150 shrink-0 touch-manipulation"
                    >
                      {time}
                    </button>
                  ))}
      ...
      {/* 🖥️ DESKTOP VIEW (md+) */}
      <div className="hidden md:block p-3.5 space-y-2.5">
        ...
        {/* 🌟 DESKTOP INLINE LIQUID GLASS QUICK-TIME DOCK */}
        {activeSuggestionConfig && !isOff && (
          <div className="flex items-center gap-3 px-3 py-1.5 rounded-xl bg-cyan-500/10 dark:bg-slate-950/80 border border-cyan-500/30 shadow-sm animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center gap-1.5 text-xs font-bold text-cyan-800 dark:text-cyan-300 shrink-0">
            ...
            <div className="flex items-center gap-1.5 overflow-x-auto whitespace-nowrap scrollbar-none">
              {activeSuggestionConfig.times.map((time) => (
                <button
                  ...
                  type="button"
                  onClick={() => handleSelectSuggestionTime(time)}
                  className="px-2.5 py-0.5 text-xs font-mono font-bold text-slate-800 dark:text-slate-100 bg-white/90 dark:bg-slate-800 hover:bg-gradient-to-r hover:from-cyan-500 hover:to-emerald-500 hover:text-white dark:hover:text-white rounded-lg border border-white/80 dark:border-slate-700 shadow-sm active:scale-95 transition-all duration-150 shrink-0"
                >
                  {time}
                </button>
              ))}
            </div>
            <span className="text-[10px] text-slate-400 ml-auto hidden sm:inline">Tap time to auto-advance →</span>
          </div>
        )}
